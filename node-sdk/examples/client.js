...
async function clientExample() {
  ...
  try {
    ...
    const syncResult = await client.executeSync('multiply', { a: 5, b: 6 }, 30000);
...
async function callExample() {
  ...
  try {
    const result1 = await call('http://localhost:8080', 'add', { a: 100, b: 200 });
    console.log('Call result 1:', result1);
    
    const result2 = await call('http://localhost:8080', 'multiply', { a: 7, b: 8 });
...
async function batchExample() {
  ...
  try {
    ...
    console.log('All tasks submitted, task IDs:', responses.map(r => r.taskId));
    console.log('Task statuses:', responses.map(r => r.status));
