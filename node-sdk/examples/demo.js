...
async function demo() {
  ...
  try {
    ...
    const greetResult = await client.executeSync(
      'greet',
      { name: 'Alice' },
      10000
    );
    ...
    const calcResult = await client.executeSync(
      'calculate',
      {
        operation: 'multiply',
        a: 15,
    ...
    const processResult = await client.executeSync(
      'processData',
      {
    ...
    const callResult1 = await call('http://localhost:8080', 'greet', {
      name: 'Bob',
    });
